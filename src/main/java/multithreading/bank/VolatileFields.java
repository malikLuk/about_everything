 ...
 * порядок следования команд. Поднобнее см JSR 133. Короче, если проще. Если записываем значение в переменную, которая
 * может быть прочитана в другом потоке, или читаем переменную, которая могла быть записана в другом потоке, то нужно
 * использовать синхронизацию. Но бывает так, что нам нужно просто, чтобы компилятор имел ввиду, что поле может быть
 * изменено. Тогда записываем его как volatile. Теперь при выполнении JVM за этим полем будет ходить в ОЗУ, а не читать
 * из регистров. Volatile - это что-то вроде упрощенной синхронизации, они НЕ гарантируют никакой атомарности операций!!!
 *
 * */

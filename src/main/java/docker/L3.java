 ...
 *      FROM python:3.7.2-alpine3.8
 *      LABEL maintainer="malik.lukmanov@gmail.com"
 *      # Устанавливаем заивисимости
 *      RUN apk add --update git
 *      # Задаем текущую рабочую директорию
 *      WORKDIR /usr/src/my_app_directory
 *      # Копируем код из локального контекста в рабочую директорию образа
 *      COPY . .
 *      # Задаем значение по умолчанию для переменной
 ...
 *      VOLUME /my_volume
 * В этом примере, кроме прочего, мы можем увидеть комментарии, начинающиеся с символа #. Одно из основных действий,
 * выполняемых средствами dockerfile - это установка пакетов. Как уже было сказано, существуют различные способы
 * установки пакетов через инструкцию RUN. Помимо стандартных линуксовых пакетных менеджеров типа apk, yum, apt-get,
 * можно использовать и pip, wheel, conda, если речь идет о пакетах python. При этом, чтобы установка была возможной,
 * нижележащий слой должен предоставить слою, в котором выполняется установка пакетов, подходящий пакетный менеджер.
 * Также, можно перечислить список пакетов в файле и передать этот файл менеджеру с помощью RUN. Обычно, таким файлам
 * дают имя requirements.txt.
 *
 * Инструкция WORKDIR.
 * Позволяет изменить рабочую директорию контейнера. С этой директорией работают инструкции COPY, ADD, RUN, CMD и
 * ENTRYPOINT, идущие за WORKDIR. Вот некоторые особенности, касающиеся этой инструкции:
 *      - Лучше устанавливать с помощью WORKDIR абсолютные пути к папкам, а не перемещаться по файловой системе с
 *              помощью команд cd в dockerfile.
 *      - Инструкция WORKDIR автоматически создает директорию в том случае, если она не существует.
 *      - Можно использовать несколько инструкций WORKDIR. Если таким инструкциям предоставляются относительные пути,
 *              то каждая из них меняет текущую рабочую диеркторию.
 *
 * Инструкция ARG.
 * Позволяет задать переменную, значение которой можно передать из командной строки в образ во время его сборки.
 * Значение для переменной по умолчанию можно представить в dockerfile. Например ARG my_var=my_default_value. В отличие
 * от ENV-переменных, ARG-переменные недоступны во время выполнения контейнера, только во время сборки. Однако, ARG-
 * переменные можно использовать для задания значений по умолчанию для ENV-переменных из командной строки в процессе
 * сборки образа. А ENV-переменные уже будут доступны в контейнере во время его выполнения.
 *
 * Инструкция ENTRYPOINT.
 * Позволяет задавать команду с аргументами, которая должна выполняться при запуске контейнера. Она похожа на команду
 * CMD, но параметры, задаваемые в ENTRYPOINT НЕ перетираются в том случае, если контейнер запускают с параметрами
 * командной строки. Вместо этого, аргументы командной строки, передаваемые в конструкции вида docker run my_image_name,
 * добавляются к аргументам, задаваемым инструкцией ENTRYPOINT. Например, после выполнения команды вида
 *      docker run my_image bash
 * аргумент bash добавится в конец списка аргументов, заданных с помощью ENTRYPOINT. Вот несколько рекомендаций:
 *      - Если при каждом запуске контейнера нужно выполнять одну и ту же команду - лучше использовать ENTRYPOINT.
 *      - Если контейнер будет использоваться в роли приложения - лучше использовать ENTRYPOINT.
 * В нашем случае, использование инструкции ENTRYPOINT ["python", "./app/my_script.py", "my_var"] приводит к тому, что
 * контейнер, при запуске стартует python-скрипт my_script.py с аргументом my_var. Обратим внимание, что в dockerfile
 * есть строка ARG my_var=my_default_value, в которой записываем значение переменной по умолчанию. Так что, если мы не
 * передадим аргумент в ENTRYPOINT, то подставится значение по умолчанию.
 *
 * Инструкция EXPOSE.
 * Указывает на то, какие порты планируется открыть для того, чтобы через них можно было связаться с работающим
 * контейнером. Эта инструкция НЕ открывает порты. А вот чтобы открыть порты, нужно выполнить команду docker run с
 * ключом -p. Если написать -P в вернем регистре, то будут открыты все порты, указанные в EXPOSE.
 *
 * Мы рассмотрели далеко не все доступные инструкции.
 *
 * */
